...
class SqliteDatabase {
  ...
  Future<bool> connect({bool reset = false}) async {
    ...
    try {
      this.database = await openDatabase(path, version: 1,
        ...
        await db.execute(
            'CREATE TABLE mood_data (mood_id INTEGER PRIMARY KEY, mood_value INTEGER, extra_data1 TEXT, date_created DATETIME DEFAULT CURRENT_TIMESTAMP)');
  ...
  Future<bool> serverSync(String uid) async {
    ...
    try {
      int logins = 0;
      try {
        logins = int.parse(await getConfig("LOGINS"));
      } catch (e) {
        logins = 1;
      }
      api.logActivity("LOGIN");
      String api_response = await api.postRequest(
              method: "getPrompts", data: logins.toString()) ??
          "";
      if (api_response == "") {
  ...
  Future<bool> create_mood_tracker_table() async {
